...
def get_dataset(dataset, group=-1, path=path_operator.get_dataset_path, args=None, res_selector=None, transforms=None,
                flatten=False, sampling_method=None, train_ratio=0.8, attr_specs=None):
    """
    ...
    if dataset == 'Office':
        ...
        print("\n使用 ResNet50 提取特征...")
        # 使用featurelize处理数据集
        train_dataset = featurelize(dataset=train_dataset)
        test_dataset = featurelize(dataset=test_dataset)
        ...
        if sampling_method is not None and sampling_method.strip():
            ...
            if num_sensitive_attrs <= 3:
                ...
                train_dataset = MultiSensitiveSamplingDataset(train_dataset, use_attention=True)

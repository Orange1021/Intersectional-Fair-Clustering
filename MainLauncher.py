...
##在深度学习中，确定性模式通常是为了确保结果可复现，比如固定随机种子，
# 禁用CUDA的某些非确定性算法。如果这个变量是False，可能意味着程序会尽可能保证结果的可重复性；
# 如果设置为True，可能允许一些非确定性的操作，比如不固定随机种子，或者使用非确定性的CUDA算法，
...
def run(use_queue=True):
    ##控制是否启用 CUDA 同步阻塞模式（用于调试，默认关闭）
    blocking = False
    ##控制是否设置 MKL_THREADING_LAYER=GNU（用于解决某些 MKL 库的线程兼容性问题，默认关闭）。
    gun = False
    if UnDeterministic:

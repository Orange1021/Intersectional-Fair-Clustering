...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", default='MNISTUSPS')  # MNISTUSPS ReverseMNIST Office MTFL
    ...
    parser.add_argument("--LambdaIntersectionalBalance", help="Weight for the new intersectional balance loss (worst-case ratio)", default=0, type=float)
    ...
    parser.add_argument('--cluster_method', type=str, default='kmeans',
                        choices=['kmeans', 'gmm', 'spectral', 'dbscan'],
                        help='聚类方法: kmeans/gmm/spectral/dbscan')

    parser.add_argument('--feature_extractor', type=str, default='resnet50',
                        choices=['resnet50', 'dinov2'],
                        help='特征提取器: resnet50/dinov2')
    ...
    if args.train_epoch >= 0:
        train_loader, test_loader, class_num, feature_dim = dataset.get_dataloader(
            dataset=args.dataset, batch_size=args.batch_size, path=path_operator.get_dataset_path, args=args, 
            sampling_method=args.sampling_method, feature_extractor=args.feature_extractor
        )
        ...
        if args.dataset == 'Office':
            net = net_edition.NetFCN(class_num=class_num, input_dim=feature_dim, group_nums=[2,2,2,3], args=args, num_sensitive_attrs=4).cuda()
        elif args.dataset == 'MTFL':
            net = net_edition.NetFCN(class_num=class_num, input_dim=feature_dim, group_nums=[2,2], args=args, num_sensitive_attrs=2).cuda()
        elif args.dataset == 'MNISTUSPS':
            net = net_edition.Net(class_num=class_num, group_nums=[2,2], args=args, num_sensitive_attrs=2).cuda()
        else:
            net = net_edition.Net(class_num=class_num, group_nums=[2,2], args=args, num_sensitive_attrs=2).cuda()

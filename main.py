...
if __name__ == "__main__":
    ...
    if args.train_epoch >= 0:
        train_loader, test_loader, class_num = dataset.get_dataloader(
            dataset=args.dataset, batch_size=args.batch_size, path=path_operator.get_dataset_path, args=args, sampling_method=args.sampling_method)
        ...
        if args.dataset == 'Office':
            net = net_edition.NetFCN(class_num=class_num, input_dim=2048, group_nums=[2,2,2,3], args=args, num_sensitive_attrs=4).cuda()
        elif args.dataset == 'HAR':
            net = net_edition.NetFCN(class_num=class_num, input_dim=561, group_nums=[2,2], args=args, num_sensitive_attrs=2).cuda()
        elif args.dataset == 'MTFL':
            net = net_edition.NetFCN(class_num=class_num, input_dim=2048, group_nums=[2,2], args=args, num_sensitive_attrs=2).cuda()
        elif args.dataset == 'MNISTUSPS':
            net = net_edition.Net(class_num=class_num, group_nums=[2,2,2,3], args=args, num_sensitive_attrs=4).cuda()
        else:
